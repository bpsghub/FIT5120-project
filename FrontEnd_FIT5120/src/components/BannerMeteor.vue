  ...
  <header class="transport-banner">
    <div class="animated-background">
      ...
      <div class="transport-icons">
        <div v-for="(icon, idx) in movingIcons" :key="idx" :class="`moving-icon moving-icon-${idx + 1}`"
          :style="{ animationDelay: `${idx * 2}s`, top: `${15 + idx * 30}%` }">
    ...
    <div class="banner-content">
      <div class="main-icon" data-aos="zoom-in" data-aos-duration="1000">
        <component v-if="mainIcon" :is="mainIcon.icon ? mainIcon.icon : mainIcon" />
      ...
      <div v-if="translatedBadges && translatedBadges.length > 0" class="transport-badges" data-aos="fade-up"
        data-aos-delay="700">
        <div class="badge" v-for="(badge, idx) in translatedBadges" :key="idx"
...
const props = defineProps({
  ...
  mainIcon: { type: [Object, Function], default: null }
})

// Computed property to translate badges
const translatedBadges = computed(() => {
  return props.badges.map(badge => ({
    ...badge,
    text: badge.translationKey ? t(badge.translationKey) : badge.text
  }))
})
...
// Default moving icons (3 transport vehicles)
const defaultMovingIcons = [
  {
    component: () => h('svg', { xmlns: 'http://www.w3.org/2000/svg', viewBox: '0 0 24 24', fill: 'currentColor' }, [
...
const movingIcons = computed(() => {
  // Use translatedBadges icons if provided, otherwise use default transport icons
  if (translatedBadges.value && translatedBadges.value.length >= 3) {
    return translatedBadges.value.slice(0, 3).map(badge => ({ component: badge.icon }))
  }
  return defaultMovingIcons
})
</script>
