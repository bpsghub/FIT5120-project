...
const sendMessage = async () => {
  ...
  try {
    ...
    while (!done) {
      ...
      console.log('Streaming chunk:', chunk); // Debug chunk output
      ...
      for (const eventRaw of events) {
        ...
        try {
          ...
          if (delta) {
            // Simple logic: check for thinking tags
            if (delta.includes('<think>')) {
              isInThinking = true;
              // Add content before <think> if any
              const thinkIndex = delta.indexOf('<think>');
              if (thinkIndex > 0) {
                fullContent += delta.substring(0, thinkIndex);
                messages.value[assistantMessageIndex].content = fullContent;
                scrollToBottom();
              }
            } else if (delta.includes('</think>')) {
              isInThinking = false;
              // Add content after </think> if any
              const endThinkIndex = delta.indexOf('</think>');
              if (endThinkIndex + 8 < delta.length) {
                fullContent += delta.substring(endThinkIndex + 8);
                messages.value[assistantMessageIndex].content = fullContent;
                scrollToBottom();
              }
            } else if (!isInThinking) {
              // Not in thinking mode, add content immediately for streaming effect
              fullContent += delta;
              messages.value[assistantMessageIndex].content = fullContent;
              scrollToBottom();
            }
            // Skip content if in thinking mode
          }
        } catch (parseError) {
