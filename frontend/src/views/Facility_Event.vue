...
import mockFacilityService from '../services/facilityService.js' // Real API
// import mockFacilityService from '../services/facilityService.js' // Mock data for now
...
const isTempMarkerActive = ref(false);
...
const loadAllFacilities = async () => {
  ...
  try {
    const response = await mockFacilityService.getAllItems()
...
const handleSearch = async () => {
  ...
  try {
    const response = await mockFacilityService.geocodeLocation(searchKeyword.value)
    facilities.value = response.data || []
...
const initializeMap = async () => {
  ...
  try {
    ...
    map.value.on('click', (e) => {
      ...
      if (isTempMarkerActive.value) {
        removeTempMarker();
      }


      const { lat, lng } = e.latlng;
      tempMarkerData.value.coordinates = { lat, lng };
...
const createTempMarker = (lat, lng) => {
  ...
  tempMarker.value = L.marker([lat, lng], { icon: tempIcon }).addTo(map.value);
  isTempMarkerActive.value = true;

  const popupContent = `
    <div style="min-width: 250px;">
      <h4 style="margin: 0 0 15px 0; color: #dc3545;">添加自定义设施</h4>
      ...
      <div style="margin-bottom: 10px; text-align: left;">
        ...
        <input
          ...
          oninput="window.updateTempMarkerData('name', this.value)"
          required
        >
      </div>
      ...
      <div style="margin-bottom: 10px; text-align: left;">
        ...
        <select
          style="width: 100%; padding: 8px; border: 1px solid #ddd; border-radius: 4px;"
          onchange="window.updateTempMarkerData('type', this.value)"
        >
          <option value="restaurant">餐厅</option>
          <option value="supermarket">超市</option>
          <option value="community center">社区中心</option>
        </select>
      </div>
      ...
      <div style="margin-bottom: 15px; text-align: left;">
        ...
        <input
          ...
          style="width: 100%; padding: 8px; border: 1px solid #ddd; border-radius: 4px;"
          oninput="window.updateTempMarkerData('address', this.value)"
        >
      </div>
  ...
  tempMarker.value.bindPopup(popupContent, { maxWidth: 300, className: 'temp-popup' }).openPopup();
};
...
const removeTempMarker = () => {
  if (tempMarker.value && map.value) {
    ...
    isTempMarkerActive.value = false;
    tempMarkerData.value = {
      name: '',
      type: 'restaurant',
      address: '',
      phone: '',
...
const updateTempMarkerData = (key, value) => {
  if (Object.prototype.hasOwnProperty.call(tempMarkerData.value, key)) {
    tempMarkerData.value[key] = value;
  }
};

const saveTempMarker = () => {
  if (!tempMarkerData.value.name.trim()) {
    alert('请输入设施名称！');
    return;
  }
  ...
  const newFacility = {
    id: Date.now(),
    ...tempMarkerData.value,
    image: 'https://via.placeholder.com/300x200?text=Custom+Facility'
  };

  // 添加到设施列表
  facilities.value.unshift(newFacility);
  console.log('✅ 保存自定义设施：', newFacility);
...
onMounted(() => {
  ...
  window.updateTempMarkerData = updateTempMarkerData;
  window.saveTempMarker = saveTempMarker;
  window.cancelTempMarker = removeTempMarker;
})
...
onUnmounted(() => {
  ...
  if (window.updateTempMarkerData) delete window.updateTempMarkerData;
  if (window.saveTempMarker) delete window.saveTempMarker;
  if (window.cancelTempMarker) delete window.cancelTempMarker;
  removeTempMarker();
})
</script>
...
:deep(.temp-marker:hover) {
  transform: scale(1.2);
}

/* 临时标注弹窗样式 */
:deep(.temp-popup .leaflet-popup-content-wrapper) {
  border: 2px solid #dc3545; /* 红色边框标识临时状态 */
  border-radius: 8px;
  box-shadow: 0 4px 12px rgba(220, 53, 69, 0.2);
}

:deep(.temp-popup .leaflet-popup-tip) {
  background-color: #dc3545;
}
